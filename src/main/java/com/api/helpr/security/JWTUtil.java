...
public class JWTUtil {
	...
	public boolean tokenValido(String token) {
		Claims claims = getClaims(token);// converte o token em uma articulação de json
		if (claims != null) {
			String username = claims.getSubject(); // coleta o email
			Date expirationDate = claims.getExpiration();
			Date now = new Date(System.currentTimeMillis());

			if (username != null && expirationDate != null && now.before(expirationDate)) {// data anterior a atual
				return true;
			}
		}
		return false;
